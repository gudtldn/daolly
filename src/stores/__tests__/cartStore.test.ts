...
describe("cartStore", () => {
  ...
  describe("submit", () => {
    it("고객 미선택 시 에러", async () => {
      useCartStore.getState().addItem(sampleItem);
      await expect(useCartStore.getState().submit("테스트")).rejects.toThrow(
    ...
    it("빈 카트 시 에러", async () => {
      ...
      await expect(useCartStore.getState().submit("테스트")).rejects.toThrow("Cart is empty");
    });

    it("성공 시 API 호출 + 카트 초기화", async () => {
      const mockWorkItem = { id: 1, customerId: 1, status: "Received" as const, description: "와이셔츠 외 1건", price: 7000, paidAmount: 0, note: null, receivedAt: "", completedAt: null, pickedUpAt: null, createdAt: "", lastModifiedAt: "" };
      ...
      const result = await useCartStore.getState().submit("와이셔츠 외 1건", "급행");

      expect(mockCreate).toHaveBeenCalledWith({
        customerId: 1,
        description: "와이셔츠 외 1건",
        price: 7000,
        note: "급행",
        details: [
          { itemName: "와이셔츠", unitPrice: 3000, quantity: 1, optionsMemo: null },
          { itemName: "바지", unitPrice: 4000, quantity: 1, optionsMemo: null },
        ],
      });
