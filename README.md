...
1. `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `package.json`의 버전을 올리고 `v*` 태그를 push합니다.
2. Release 워크플로가 CI(타입 검사·테스트·린트)를 먼저 실행하고, 통과하면 **드래프트 릴리스**를 만듭니다.
3. 드래프트의 설치 파일을 직접 설치해 실제 데이터 사본으로 기동·마이그레이션을 확인합니다.
4. 확인이 끝나면 드래프트를 게시합니다. 게시한 뒤에야 설치된 앱의 자동 업데이트에 노출됩니다.
